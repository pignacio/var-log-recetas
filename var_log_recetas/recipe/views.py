...
from var_log_recetas.ingredient.models import MeasureUnit
from var_log_recetas.utils.views import has_get_models, has_models

from .forms import (
    MeasuredIngredientForm,
    RecipeAddForm,
    StepForm,
    SubRecipeForm,
    SubRecipeSelectForm,
)
from .models import MeasuredIngredient, Recipe, Step, SubRecipe

logger = logging.getLogger(__name__)  # pylint: disable=invalid-name


def _render_json_error(err, *_args, **_kwargs):
    return HttpResponse(json.dumps({"success": False, "error": str(err)}))
...
class RecipeEditView(RedirectView):
    ...
    def get_redirect_url(self, *args, **kwargs):
        ...
        try:
            ...
            subrecipe = recipe.subrecipe_set.first() or recipe.subrecipe_set.create(
                title="la receta"
            )
...
class SubRecipeRenameView(FormView):
    ...
    def get_form(self, form_class):
        subrecipe = get_object_or_404(SubRecipe, id=self.kwargs.get("subrecipe_id"))
        return form_class(instance=subrecipe, **self.get_form_kwargs())
...
class SubRecipeDetailView(DetailView):
    ...
    def get_context_data(self, *args, **kwargs):
        context = super(SubRecipeDetailView, self).get_context_data(*args, **kwargs)
        if self.object.recipe.subrecipe_set.count() > 1:
            context["subrecipe_select_form"] = SubRecipeSelectForm(
                self.object.recipe, initial=dict(subrecipe=self.object.id),
            )
...
def home(request):
    return render(
        request, "recipe/home.html", {"recipes": Recipe.objects.all().order_by("id")}
    )
...
def subrecipe_edit(request, subrecipe):
    return render(
        request,
        "recipe/subrecipe_edit.html",
        {"recipe": subrecipe.recipe, "subrecipe": subrecipe},
    )
...
def subrecipe_edit_ingredient_add(request, subrecipe):
    ...
    return render(request, "recipe/recipe_edit/ingredient_add.html", {"form": form})


@has_models
def subrecipe_edit_ingredient_delete(
    request, subrecipe
...
def subrecipe_edit_ingredients(request, subrecipe):
    return render(
        request,
        "recipe/recipe_edit/ingredients.html",
...
def subrecipe_edit_step_add(request, subrecipe):
    ...
    return render(request, "layout/crispyform.html", {"form": form})


@has_get_models("step", on_error=_render_json_error)
def subrecipe_edit_step_delete(
    _request, subrecipe, step
...
def subrecipe_edit_steps(request, subrecipe):
    return render(
        request, "recipe/recipe_edit/steps.html", {"steps": subrecipe.step_set.all()}
    )
...
def subrecipe_edit_step_move_up(_request, subrecipe, step):
    error = None
    previous_step = (
        subrecipe.step_set.filter(position__lt=step.position)
        .order_by("-position")
        .first()
    )
...
def subrecipe_edit_step_move_down(_request, subrecipe, step):
    error = None
    next_step = (
        subrecipe.step_set.filter(position__gt=step.position)
        .order_by("position")
        .first()
    )
...
def recipe_export(_request, recipe):
    ...
    response = HttpResponse(
        json.dumps(recipe.as_json(), indent=1), content_type="application/json"
    )
...
def recipe_export_all(_request):
    ...
    response = HttpResponse(
        json.dumps(output, indent=1, sort_keys=True), content_type="application/json"
    )
    response["Content-Disposition"] = "attachment; filename={}".format(filename)
    return response
